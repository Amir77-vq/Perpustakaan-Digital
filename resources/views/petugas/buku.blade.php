...
<script src="{{ asset('assets/js/validation-buku.js') }}"></script>
      ...
      <div class="card shadow-sm border-0" style="border-radius: 15px; background: #fff;">
        ...
        <div class="mx-3 position-relative z-index-2">
          <div class="px-4 d-flex align-items-center justify-content-between" 
            ...
            {{-- FIX: Nama route di web.php adalah 'buku.create' --}}
            <a href="{{ route('buku.create') }}" class="btn btn-sm mb-0" 
              ...
              <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #f2f2f2;">
                      <td class="ps-4">
                        <span class="text-xs font-weight-bold text-dark">{{ str_pad($buku->id, 2, '0', STR_PAD_LEFT) }}</span>
                      </td>
                      <td class="ps-2">
                        <p class="text-sm font-weight-bold mb-0" style="color: #344767;">{{ $buku->judul }}</p>
                      </td>
                      <td class="ps-2">
                        <span class="text-xs text-secondary font-weight-normal">{{ $buku->penulis }}</span>
                      </td>
                      <td class="align-middle text-center">
                        <span class="text-xs font-weight-bold text-secondary">{{ $buku->stok }}</span>
                      </td>
                      <td class="align-middle text-center">
                        <div class="d-flex justify-content-center gap-2">
                          
                          {{-- FIX: Route Edit adalah 'buku.edit' --}}
                          <a href="{{ route('buku.edit', $buku->id) }}" class="btn btn-xs mb-0 px-3 py-1" 
                              style="background-color: #2152ff; color: #fff; font-size: 10px; border-radius: 5px; font-weight: 700; border: none;">
                              EDIT
                          </a>
                          
                          {{-- FIX: Route Destroy adalah 'buku.destroy' --}}
                          <form action="{{ route('buku.destroy', $buku->id) }}" method="POST" id="form-hapus-{{ $buku->id }}">
                              @csrf
                              @method('DELETE')
                              <button type="button" class="btn btn-xs mb-0 px-3 py-1 btn-hapus" data-id="{{ $buku->id }}"
